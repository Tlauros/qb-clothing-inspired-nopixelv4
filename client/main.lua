    ...
    CreateThread(function()
        ...
        for k, v in pairs(Config.ClothingRooms) do
            ...
            if v.isGang then
                ...
                action = function()  ----- in jast bara job haa
                    customCamLocation = v.cameraLocation
        ...
        clothingCombo:onPlayerInOut(function(isPointInside, _, zone)
            if isPointInside then
                ...
                if zoneName == 'surgeon' then
                    ---displayTextUI("Plastic Surgery", "E")
                    exports['nc-textui']:displayTextUI("Plastic Surgery", "E")
                elseif zoneName == 'clothing' then
                    exports['nc-textui']:displayTextUI("clothing", "E")
                elseif zoneName == 'barber' then
                    exports['nc-textui']:displayTextUI("barber", "E")
                end
            else
                inZone = false
                exports['nc-textui']:hideTextUI()
            end
        end)
...
local function getPedDecorationType() --- miyad jensiyat player ro dar miyare (sarenakh)
...
local function CheckingConfigBlacklist()
    local List_Of_BlackList = {}

    local gender = getPedDecorationType()
    local Blacklist = Config.Blacklist[gender]

    if Blacklist then
        -- print("we recognize player and must open: " .. gender .. " Blacklist!")

        for category, items in pairs(Blacklist.components) do
            table.insert(List_Of_BlackList, { category = category, items = items })
        end

        -- for index, data in ipairs(List_Of_BlackList) do
        --     print("Index:", index)
        ...
        --     print("------")
        -- end
        return List_Of_BlackList
    end
end

function openMenu(allowedMenus)
    local List_BlackList = CheckingConfigBlacklist()
    previousSkinData = json.encode(skinData)
    creatingCharacter = true
    exports.vip_cars:ICH(creatingCharacter)
    local CheckGenderPed = getPedDecorationType()
    PlayerData = QBCore.Functions.GetPlayerData()
    local trackerMeta = PlayerData.metadata["tracker"]

    GetMaxValues()
    SendNUIMessage({    -------- in mohem tare chon kole data mifreste samte NUI
        action = "open",
        menus = allowedMenus,
        currentClothing = skinData,
        hasTracker = trackerMeta,
        Gender = CheckGenderPed,
...
RegisterNUICallback('updateSkin', function(data, cb)   --- motavejeh shodm besorate halqe binahayet optimze shode kar mikone ke befamon shod (sarenakh)
